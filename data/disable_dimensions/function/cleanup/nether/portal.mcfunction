# Detect world-edge cases automatically in a 5x5x5 cube around the player/ender pearl
...
# Otherwise use the normal centered 5×5×5 (-2..+2)
execute unless predicate disable_dimensions:below_world_floor unless predicate disable_dimensions:above_world_ceiling run execute store success storage disable_dimensions:tmp removed byte 1 run fill ~-2 ~-2 ~-2 ~2 ~2 ~2 air replace minecraft:nether_portal
