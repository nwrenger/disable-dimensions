...
data remove storage disable_dimensions:tmp had_config
data remove storage disable_dimensions:tmp load_reset
data remove storage disable_dimensions:tmp load_repaired

# Only show load warnings when some config content already existed.
execute if data storage disable_dimensions:config world_spawn \
    run data modify storage disable_dimensions:tmp had_config set value 1b
execute if data storage disable_dimensions:config dimensions \
    run data modify storage disable_dimensions:tmp had_config set value 1b

# Repair world spawn config
...
# Reset fully when the dimensions list is missing
execute unless data storage disable_dimensions:config dimensions[] \
...
execute if data storage disable_dimensions:tmp {load_reset:1b} \
    if data storage disable_dimensions:tmp {had_config:1b} \
